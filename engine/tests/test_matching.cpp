...
TEST_CASE("A non-crossing limit order rests and updates the best bid") {
    Book book;
    const auto result =
        book.add_order({1, 10, Side::Buy, OrderType::Limit, 100, 5}, 1);
...
TEST_CASE(
    "A crossing limit order executes at the passive price and removes it") {
    Book book;
    book.add_order({1, 10, Side::Sell, OrderType::Limit, 100, 5}, 1);

    const auto result =
        book.add_order({2, 20, Side::Buy, OrderType::Limit, 105, 5}, 2);
    ...
    CHECK(result.trades[0].buy_owner == 20);
    CHECK(result.trades[0].sell_owner == 10);
    CHECK(result.trades[0].aggressor_side == Side::Buy);
    CHECK(result.trades[0].ts == 2);
    CHECK(result.unaccepted_quantity == 0);
    CHECK(result.reject_reason == RejectReason::None);
    CHECK_FALSE(book.best_ask().has_value());
}
...
TEST_CASE("A partial fill leaves only the resting remainder") {
    ...
    const auto first =
        book.add_order({2, 20, Side::Buy, OrderType::Limit, 100, 4}, 2);
    const auto second =
        book.add_order({3, 30, Side::Buy, OrderType::Limit, 100, 6}, 3);
...
TEST_CASE("Matching walks price levels and same-level orders in FIFO order") {
    ...
    const auto result =
        book.add_order({5, 99, Side::Buy, OrderType::Limit, 101, 12}, 5);
...
TEST_CASE("Invalid limit prices and quantities are rejected before matching") {
    ...
    CHECK(book.add_order({1, 1, Side::Buy, OrderType::Limit, 0, 1}, 1)
              .reject_reason == RejectReason::InvalidPrice);
    CHECK(book.add_order({2, 1, Side::Buy, OrderType::Limit, -1, 1}, 2)
              .reject_reason == RejectReason::InvalidPrice);
...
TEST_CASE("A partially filled limit order reports no remaining quantity after "
          "resting") {
    Book book;
